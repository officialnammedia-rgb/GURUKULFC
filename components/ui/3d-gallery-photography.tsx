...
function GalleryScene({
	...
	// Handle pointer drag input (mouse & touch)
	...
	const handlePointerMove = useCallback(
		(event: PointerEvent) => {
			...
			const deltaY = event.clientY - previousPointerPos.current.y;
			const deltaX = event.clientX - previousPointerPos.current.x;
			const dragDelta = (deltaY * 0.8 - deltaX * 0.5) * speed * 0.08;
			setScrollVelocity((prev) => prev + dragDelta);
			previousPointerPos.current = { x: event.clientX, y: event.clientY };
			lastInteraction.current = Date.now();
		},
		[speed]
	...
	useEffect(() => {
		...
		const parentContainer = domEl.closest('.victories-canvas-wrapper') || domEl.parentElement;
...
function FallbackGallery({ images }: { images: ImageItem[] }) {
	...
	return (
		<div className="flex flex-col items-center justify-center h-full bg-gray-900 p-4 rounded-2xl">
